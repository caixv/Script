...
****************************
[Script]
Bili Region = type=http-response,pattern=^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/offline\/version)\?,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js

#可选, 适用于搜索指定地区的番剧
Bili Search = type=http-request,pattern=^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)&,script-path=https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js
...
****************************
[rewrite_local]
^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/offline\/version)\? url script-response-body https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js

#可选, 适用于搜索指定地区的番剧
^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)& url script-request-header https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js
...
****************************
[Script]
http-response ^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/offline\/version)\? script-path=https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js, requires-body=true, tag=bili自动地区

#可选, 适用于搜索指定地区的番剧
http-request ^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)& script-path=https://raw.githubusercontent.com/caixv/Script/master/Surge/JS/Bili_Auto_Regions.js, tag=bili自动地区(搜索)
...
async function SwitchRegion(play) {
	const Group = $.read('Bilibili') || 'DIRECT'; //Your blibli policy group name.
	const CN = $.read('Bilibili') || 'DIRECT'; //Your China sub-policy name.
	const TW = $.read('Bilibili') || '🇨🇳 V3-PRO 台湾03 Hinet | 8x 原生IP'; //Your Taiwan sub-policy name.
	const HK = $.read('Bilibili') || '🇵🇷 V3-PRO 香港HKBGP01 | 8x'; //Your HongKong sub-policy name.
	const DF = $.read('Bilibili') || 'DIRECT'; //Sub-policy name used after region is blocked(e.g. url 404)
